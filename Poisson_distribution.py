...
Generate random values with poisson distribution and dynamically
graph their frequencies.
...
def update_frame(frame_number, event_count, values, frequencies):
    ...
    #checking out of range random values
    for _ in range(event_count):
        while True:
            index=np.random.poisson(5)
    ...
    show_bar_chart(f' Frequencies of {freq_sum:,} Poissonally Distrbuted Random Values ' +
...
values = list(range(1, 19))  # displaying values on y-axis
frequencies = [0]*20        #  list of frequencies of values

# Configure and start the animation that calls function update_frame.

poisson_animation = animation.FuncAnimation(figure, update_frame,repeat=False,
